...
/// ファイター1CPU操作
/// </summary>
public partial class CpuFighter1 : CpuFighterRoot
{
    private RayCast2D _waza1Ray;
    ...
    {
        base._Ready();
        _waza1Ray = GetNode<RayCast2D>("Waza1Ray");
    ...
    public override void _Process(double delta)
    {
        ...
        {
            // 必殺技2の射程距離
            if (m_Random.Randf() <= Waza2Probability && _waza2Area.HasFighter(FighterId) is not null)
            {
                InsertWaza2();
                return;
            }
            ...
            // 必殺技3の射程距離
            if (m_Random.Randf() <= Waza1Probability && _waza3Area.HasFighter(FighterId) is not null)
            {
                InsertWaza3();
                return;
            }
        ...
        {
            // 必殺技1の射程距離
            if (m_Random.Randf() <= Waza1Probability && _waza1Ray.GetCollider() is FighterRoot)
            {
                InsertWaza1();
                return;
            }
    ...
    {
        InsertBuffer(Fighter.RightSide ? WazaKey.Right : WazaKey.Left);
