...
/// ファイター3CPU操作
/// </summary>
public partial class CpuFighter3 : CpuFighterRoot
{
    private SightArea _waza1Area;
    private RayCast2D _waza2Ray;
    ...
    public override void _Process(double delta)
    {
        ...
        if (m_Random.Randf() <= Waza3Probability)
        {
            Waza3Probability = -1f;
            m_NextKey.Clear();
            InsertWaza3();
        ...
        {
            // 必殺技2の射程距離
            if (m_Random.Randf() <= Waza2Probability && _waza2Ray.IsColliding())
            {
                InsertWaza2();
                return;
            }
    ...
    {
        int r = Fighter.RightSide ? WazaKey.Left : WazaKey.Right;
        int l = Fighter.RightSide ? WazaKey.Right : WazaKey.Left;
        InsertBuffer(r);
