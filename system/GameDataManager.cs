...
{
    public const int NullSlotNo = int.MinValue;
    ...
    [GeneratedRegex("^/root/.*?/")]
    private static partial Regex RootNodeRegex();
    ...
    public Error Load(int slotNo)
    {
        StartNewGame();
        string file = string.Format(DataFilePath, slotNo);
        Error e = UseEncriptData ? _file.LoadEncryptedPass(file, Password) : _file.Load(file);
    ...
    public Error LoadInitialStartData()
    {
        StartNewGame();
        return Error.Ok;
    }
    ...
    public Error Save(int slotNo)
    {
        ...
        _packData.RemoveIllegalKey(_file);
        string file = string.Format(DataFilePath, slotNo);
        e = UseEncriptData ? _file.SaveEncryptedPass(file, Password) : _file.Save(file);
    ...
    public string GetFileDate(int slotNo)
    {
        ...
        Error e = UseEncriptData ? data.LoadEncryptedPass(file, Password) : data.Load(file);
